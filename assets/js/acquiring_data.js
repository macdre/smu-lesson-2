...
//$('#mw-content-text').children('table').eq(3).addClass('mytable');

// Get indexes; we will be using these a lot!
// Discuss: _.map, .eq(0), obj + iter, .text()
indexKeys = _.map($('.mytable').children('thead').find('th'), 
            function(obj){
...
var bigmap = _.map($('.mytable tbody').children('tr'), function(obj,iter){
    var vals = _.map($(obj).children(), function(obj2,iter2){
        var value = iter2 == 0 ? $(obj2).text().trim() : parseFloat($(obj2).text().replace(/\,|r/g, ''))
        //console.log(value)
        return value;
    }); //innermap
...
var atlanticCanada = _.filter(bigmap, function(obj,iter){
    if(obj['Geography']){
        return obj['Geography'].indexOf('[1') > -1;
    }
})

var quebec = _.filter(bigmap, function(obj,iter){
    if(obj['Geography']){
        return obj['Geography'].indexOf('[2') > -1;
    }
})

var ontario = _.filter(bigmap, function(obj,iter){
    if(obj['Geography']){
        return obj['Geography'].indexOf('[3') > -1;
    }
})

var praries = _.filter(bigmap, function(obj,iter){
    if(obj['Geography']){
        return obj['Geography'].indexOf('[4') > -1;
    }
})

var bc = _.filter(bigmap, function(obj,iter){
    if(obj['Geography']){
        return obj['Geography'].indexOf('[5') > -1;
    }
})

// And include them in a large array of objects:
var hugemap = [
  {"name": "atlanticCanada", "values": atlanticCanada},
  {"name": "quebec", "values": quebec},
  {"name": "ontario", "values": ontario},
  {"name": "praries", "values": praries},
  {"name": "bc", "values": bc}
]
...
JSON.stringify(comparable, function(key, val) {
